    ...
    if request.method == 'POST':
        ...
        try:
            if user_type == "admins":
                while True:
                    try:
                        df = pd.read_excel("./static/admin/adminlogin.xlsx")
                        time.sleep(0.1)
                    except Exception as e:
                        print(f"Error: {e}")
                        # Handle the error (e.g., log, retry, or break out of the loop)
...
def admin_ui():
    ...
    if admin:
        ...
        while True:
            try:
                # Read folder names from the 'db' directory
...
def admin_management():
    ...
    if admin == True:
        ...
        try:
            ...
            df = df.drop_duplicates(subset=['admin_id'], keep='first')
            
            # Sort the DataFrame by admin_id
...
def facultyfb():
    ...
    if session['userid'] == True:
        if request.method == 'POST':
            ...
            names_to_remove = ['RollNumber', 'Password', 'Feedback', 'Name', 'MobileNumber', 'Email', 'Suggestion']
        ...
        try:
            ...
            # Specify names to be removed
            names_to_remove = ['RollNumber', 'Password', 'Feedback', 'Suggestion', 'Name', 'MobileNumber', 'Email']
